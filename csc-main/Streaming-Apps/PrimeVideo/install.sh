...
if [ -f "$LOCAL_CHROME_SCRIPT" ]; then
	...
	if curl -fsSL https://raw.githubusercontent.com/SIMARSINGHRAYAT/LINUX-DESKTOP-APPS/main/csc-main/scripts/install-chrome-streaming-app.sh | sh -s -- io.github.example.PrimeVideoDesktop 'Prime Video' https://www.primevideo.com/ prime-video-desktop prime.png; then exit 0; fi
fi
