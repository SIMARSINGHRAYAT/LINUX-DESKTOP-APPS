...
if [ -f "$LOCAL_CHROME_SCRIPT" ]; then
	...
	if curl -fsSL https://raw.githubusercontent.com/SIMARSINGHRAYAT/LINUX-DESKTOP-APPS/main/csc-main/scripts/install-chrome-streaming-app.sh | sh -s -- io.github.example.NetflixDesktop Netflix https://www.netflix.com/ netflix-desktop netflix.png; then exit 0; fi
fi
