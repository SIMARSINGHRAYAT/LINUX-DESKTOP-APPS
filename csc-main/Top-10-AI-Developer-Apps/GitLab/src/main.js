...
const APP_ID = 'io.github.example.GitLabDesktop';
const APP_URL = 'https://gitlab.com/';
const PARTITION = 'persist:gitlab';
const APP_HOSTS = new Set(['gitlab.com', 'www.gitlab.com', 'registry.gitlab.com', 'gitlab.io', 'gitlabusercontent.com']);
...
function configurePolicies(window) {
  ...
  contents.setWindowOpenHandler(({ url }) => {
    ...
    return { action: 'allow', overrideBrowserWindowOptions: { width: 1100, height: 800, title: 'GitLab Desktop', icon: path.join(__dirname, '..', 'resources', 'gitlab-desktop.png'), webPreferences: { preload: path.join(__dirname, 'preload.js'), contextIsolation: true, nodeIntegration: false, sandbox: true, webSecurity: true, partition: PARTITION } } };
  });
...
function createMenu() {
  Menu.setApplicationMenu(Menu.buildFromTemplate([
    ...
    { role: 'windowMenu' },
    { role: 'help', submenu: [{ label: 'GitLab.com', click: () => mainWindow?.loadURL(APP_URL) }] }
  ]));
}

async function createWindow() {
  statePath = path.join(app.getPath('userData'), 'window-state.json');
  mainWindow = new BrowserWindow({ ...readWindowState(), minWidth: 640, minHeight: 480, title: 'GitLab Desktop', icon: path.join(__dirname, '..', 'resources', 'gitlab-desktop.png'), webPreferences: { preload: path.join(__dirname, 'preload.js'), contextIsolation: true, nodeIntegration: false, sandbox: true, webSecurity: true, partition: PARTITION } });
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(async () => {
    const gitlabSession = session.fromPartition(PARTITION);
    gitlabSession.setUserAgent(app.userAgentFallback.replace(/\sElectron\/[^\s]+/, ''));
    gitlabSession.setPermissionRequestHandler((_webContents, permission, callback) => callback(permission === 'notifications'));
    createMenu();
    await createWindow();
